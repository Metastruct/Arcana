...
if CLIENT then
	...
	function ENT:DrawDistortionRing(pos, innerRadius, outerRadius)
		...
		for i = 1, 12 do
			...
			local distortionPos = pos + Vector(math.cos(angle) * innerRadius * 1.3, math.sin(angle) * innerRadius * 1.3, 0)
	...
	function ENT:DrawAccretionDisk(pos, radius, eventHorizonRadius)
		...
		for i = 0, 8 do
			...
			for j = 0, segments do
				...
				points[j + 1] = pos + Vector(math.cos(angle) * currentRadius, math.sin(angle) * currentRadius, math.sin(CurTime() + i * 0.3) * diskThickness * 0.2) -- Small z-variation
			end
			...
			for j = 1, segments do
				render.DrawBeam(points[j], points[j + 1], width, 0, 1, ringColor)
			end

			-- Connect the last point back to the first to complete the circle
			render.DrawBeam(points[segments + 1], points[1], width, 0, 1, ringColor)
		end
		...
		for i = 0, 5 do
			...
			for j = 0, segments do
				...
				points[j + 1] = pos + Vector(math.cos(angle) * currentRadius, math.sin(angle) * currentRadius, math.cos(CurTime() * 1.3 + i) * diskThickness * 0.15) -- Different z-variation
			end
			...
			for j = 1, segments do
				render.DrawBeam(points[j], points[j + 1], width, 0, 1, offsetColor)
			end

			render.DrawBeam(points[segments + 1], points[1], width, 0, 1, offsetColor)
		end
		...
		for i = 0, 3 do
			...
			for j = 0, segments do
				...
				points[j + 1] = pos + Vector(math.cos(angle) * currentRadius, math.sin(angle) * currentRadius, math.sin(CurTime() * 0.7 - i) * diskThickness * 0.25) -- Different pattern
			end
			...
			for j = 1, segments do
				render.DrawBeam(points[j], points[j + 1], width, 0, 1, counterColor)
			end

			render.DrawBeam(points[segments + 1], points[1], width, 0, 1, counterColor)
		end
		...
		for i = 1, 8 do
			...
			local flarePos = pos + Vector(math.cos(flareAngle) * flareRadius, math.sin(flareAngle) * flareRadius, math.sin(CurTime() + i) * diskThickness * 0.3)
			-- Bright pulsing flares
			local flareSize = radius * 0.3 * (0.8 + 0.2 * math.sin(CurTime() * 3 + i))
			local flareColor = i % 2 == 0 and Color(170, 200, 255, 150) or Color(140, 80, 240, 150) -- Blue -- Purple
			render.SetMaterial(FLARE_MATERIAL)
		...
		for i = 1, 12 do
			...
			local startPos = pos + Vector(math.cos(beamAngle) * innerRadius * 1.2, math.sin(beamAngle) * innerRadius * 1.2, 0)
			local endPos = pos + Vector(math.cos(beamAngle) * outerRadius * (0.8 + 0.2 * math.sin(CurTime() + i)), math.sin(beamAngle) * outerRadius * (0.8 + 0.2 * math.sin(CurTime() + i)), math.sin(CurTime() * 1.5 + i) * diskThickness * 0.4)
			-- Draw radial beam
			render.SetMaterial(STREAK_MATERIAL)
			local beamWidth = radius * 0.12 * (0.7 + 0.3 * math.sin(CurTime() * 2 + i))
			local beamColor = i % 3 == 0 and Color(160, 190, 255, 100) or Color(130, 70, 230, 100) -- Blue -- Purple
	...
	function ENT:DrawEnergyJets(pos, radius, eventHorizonRadius)
		...
		-- Draw main jet beams
		render.DrawBeam(jetBaseUp, jetBaseUp + Vector(0, 0, jetLength), jetWidth * pulseScale, 0, 1, jetColor)
		render.DrawBeam(jetBaseDown, jetBaseDown + Vector(0, 0, -jetLength), jetWidth * pulseScale, 0, 1, jetColor)
	...
	function ENT:DrawDustClouds(pos, radius, eventHorizonRadius)
		...
		for i = 1, 10 do
			...
			local cloudPos = pos + Vector(math.cos(angle) * cloudDist, math.sin(angle) * cloudDist, math.sin(CurTime() + i) * radius * 0.5)
			-- Varied cloud colors
			local cloudColor
	...
	function ENT:DrawLightStreaks(pos, radius, eventHorizonRadius)
		...
		for i = 1, 12 do
			...
			local startPos = pos + Vector(math.cos(angle1) * startRadius, math.sin(angle1) * startRadius, math.sin(CurTime() + i) * radius * 0.2)
			local endPos = pos + Vector(math.cos(angle2) * endRadius, math.sin(angle2) * endRadius, math.cos(CurTime() + i) * radius * 0.3)
			-- Streak color
			local streakColor
	...
	function ENT:DrawDebris(pos, radius, eventHorizonRadius)
		...
		if self.NextDebris and CurTime() > self.NextDebris then
			...
			for i = 1, 3 do
				...
				local debrisPos = pos + Vector(math.cos(rad) * distance, math.sin(rad) * distance, math.Rand(-radius, radius) * 0.5)
				-- Create the particle
	...
	function ENT:DrawParticles(pos, radius, eventHorizonRadius)
		...
		if CurTime() > self.NextParticle then
			...
			for i = 1, particleCount do
				...
				-- Calculate position
				local particlePos = pos + Vector(math.cos(rad) * distance, math.sin(rad) * distance, math.Rand(-distance * 0.1, distance * 0.1)) -- Small z variation
				-- Create the particle
...
if CLIENT then
	...
	function EFFECT:Think()
		...
		for i = 1, 2 do
			...
			local startPos = self.Position + Vector(math.cos(rad) * distance, math.sin(rad) * distance, height)
			-- Create the particle
