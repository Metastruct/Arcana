...
for /f "usebackq tokens=* delims=" %%A in ("%File%") do (
    ...
    REM Move everything from %CD%/src/shaders/fxc to %CD%/shaders/fxc
    if exist "%CD%/src/shaders/fxc" (
        mkdir "%CD%/shaders/fxc" >nul 2>&1
        move /Y "%CD%/src/shaders/fxc\*" "%CD%/shaders/fxc" >nul
        REM Optionally, remove the src/shaders/fxc folder if empty
        rmdir "%CD%/src/shaders/fxc" 2>nul
    )
)
